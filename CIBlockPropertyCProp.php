...
class CIBlockPropertyCProp 
{
    ...
    function showHTMLElement ($code, $title, $arValue, $strHTMLControlName) 
    {	
        $result = '';
        $v = !empty($arValue['VALUE'][$code]) ? $arValue['VALUE'][$code] : '';
        $result .= '<tr>
                    <td align="right">'.$title.': </td>
                    <td>'.CFileMan::AddHTMLEditorFrame(
...
//<input type="string" value ="'.$v.'" name="'.$strHTMLControlName['VALUE'].'['.$code.']">

                /* */
        return $result;

   // echo '<pre>'; print_r($arValue['VALUE'][$code]); echo '</pre>';
	}
